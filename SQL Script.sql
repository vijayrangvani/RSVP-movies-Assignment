...
SELECT TABLE_NAME as 'Table_Name', TABLE_ROWS as 'Total No. of Rows'
...
SELECT COUNT(*) AS 'Total Number of Single Genre Movies' from one_genre_movie;
...
SELECT MIN(avg_rating) AS min_avg_rating,
       ...
       MIN(total_votes) AS min_total_votesg,
...
SELECT * FROM prod_company WHERE prod_company_rank = 1;
...
ON m.id = r.movie_id
WHERE country LIKE '%USA%' AND total_votes > 1000  AND Month(date_published) = 3 AND year = 2017
GROUP BY genre
ORDER BY movie_count DESC;
...
WITH top_genre AS
(
	SELECT m.id,g.genre,COUNT(m.id) AS movie_count
	FROM movie AS m
	INNER JOIN ratings AS r
	ON m.id = r.movie_id
	INNER JOIN genre AS g
...
) AS prod_comp WHERE prod_comp.prod_comp_rank<=3;
...
(
	...
	GROUP BY name
	HAVING COUNT(m.id) >= 3
) AS actress_ranking WHERE actress_ranking.actress_rank <= 5;
